...
/* connect_to_tracker: convenience wrapper */
...
/* send_list: sends <REQ LIST> to the tracker and prints the response.
   Uses recvLine so each line is processed individually */
...
/* send_get: sends <GET filename.track> to the tracker and saves the
   response to cache/filename. Uses recvLine to process one line at a time 
...
static void *update_thread(void *arg)
{
    ...
    while (1) {
        ...
        for (int i = 0; i < count; i++) {
            char end_byte_str[32];
            long fs = atol(snap[i].filesize);
            snprintf(end_byte_str, sizeof(end_byte_str), "%ld",
                     fs > 0 ? fs - 1 : 0);
            send_updatetracker(snap[i].filename, "0", end_byte_str);
        }
    }
    return NULL;
}
